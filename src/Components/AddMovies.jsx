...
function AddMovies() {
    ...
    alert("sorry you didn't delete the movie because you need developer permission")
      }
        ...
        onSubmit: async(values) =>{
           ...
           try{
            ...
            toast.success(result.data.message);
              ...
              delete values._id
              let result = await axios.put(`${Config.api}/movies/${currentMovie._id}`,values);
              fetchMovieData();
             formik.resetForm();
            toast.success(result.data.message);
      ...
      moviesListAll.map((m,index) => {
        return (
          <tr key={index}>
            ...
            <td>{m.trailer}</td>
            <td>
              <button  className="btn btn-primary btn-sm my-1 mx-1" onClick={()=>handleEditMovie(m._id)}>Edit</button>
              <button  className="btn btn-danger btn-sm mx-1 my-1" onClick={()=>handleDeleteMovie(m._id)}>Delete</button>
...
<div className="row my-3">
  ...
  <label htmlFor="inputText1">Movie Name : </label>
    <input type="text" placeholder="Movie name"  className={`form-control my-1 ${formik.errors.name ? "error-border" : ""}`}  name="name"
  ...
  <label htmlFor="inputText2">Movie Poster : </label>
    <input type="text" placeholder="Poster link"  className={`form-control my-1 ${formik.errors.poster ? "error-border" : ""}`}  name="poster"
         onChange={formik.handleChange}
         value={formik.values.poster} />
          {
            formik.errors.poster ? <div style={{color:"red"}}>{formik.errors.poster}</div> : null
          }
  </div> 
  <div className="col">
  <label htmlFor="inputText3">Movie Trailer : </label>
      
    <input type="text"  placeholder="Youtube link" className={`form-control my-1 ${formik.errors.trailer ? "error-border" : ""}`}  name="trailer"
         onChange={formik.handleChange}
         value={formik.values.trailer}/>
          {
            formik.errors.trailer ? <div style={{color:"red"}}>{formik.errors.trailer}</div> : null
          }
 ...
 <div className="row my-3">
  ...
  <label htmlFor="inputText4">Rating : </label>
      ...
      <input type="text"  placeholder="1" className={`form-control my-1 ${formik.errors.rating ? "error-border" : ""}`}  name="rating"
           onChange={formik.handleChange}
...
<select  id="exampleFormControlSelect2" 
       className={`form-control my-1 ${formik.errors.screen ? "error-border" : ""}`}  name="screen"
         onChange={formik.handleChange}
...
{
            formik.errors.screen ? <div style={{color:"red"}}>{formik.errors.screen}</div> : null
          }
</div>
  </div>
...
<select  id="exampleFormControlSelect3" className={`form-control my-2 ${formik.errors.certificate ? "error-border" : ""}`}  name="certificate"
         onChange={formik.handleChange}
...
{
            formik.errors.certificate ? <div style={{color:"red"}}>{formik.errors.certificate}</div> : null
          }
</div>
  </div>
...
<div className="row my-3">
  ...
  <label htmlFor="inputText5">Languages : </label>
    <input type="text" placeholder="Tamil , English"  className={`form-control my-1 ${formik.errors.languages ? "error-border" : ""}`}  name="languages"
         onChange={formik.handleChange}
...
<textarea  id="exampleFormControlTextarea1" rows={3}  className={`form-control my-2 ${formik.errors.summary ? "error-border" : ""}`}  name="summary"
         onChange={formik.handleChange}
